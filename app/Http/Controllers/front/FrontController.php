...
class FrontController extends Controller
{
    ...
    public function product(Request $request, $slug)
    {
        ...
        $categoryId = $result['product'][0]->category_id ?? 0;
    ...
    public function add_to_cart(Request $request)
    {
        ...
        $result = DB::table('cart')
            ...
            ->where(['user_id' => $uid])
            ->where(['user_type' => $user_type])
            ->select('cart.qty', 'products.name', 'products.image', 'products_attr.price', 'products.slug', 'products.id as pid', 'products_attr.id as attr_id')
            ->get();
            dd($uid);
    ...
    public function registration_process(Request $request)
    {
       ...
       if(!$valid->passes()){
            ...
            if($query){
                ...
                return response()->json(['status'=>'success','msg'=>"Registration successfully. Please check your email id for varification"]);
...
public function forgot_password(Request $request)
{
    ...
    Mail::send('front/forgot_email',$data,function($messages)
     use ($user){
        $messages->to($user['to']);
...
public function place_order(Request $request)
{
    ...
    public function order_detail(Request $request,$id)
    {
        $result['orders_details']=
                ...
                ->select('orders.*','orders_details.price','orders_details.qty','products.name as pname','products_attr.attr_image','orders_status.orders_status')
                ->leftJoin('orders','orders.id','=','orders_details.orders_id')
                ->leftJoin('products_attr','product_attr.id','=','orders_details.product_attr_id')
                ->leftJoin('products','products.id','=','product_attr.products_id')
                ->leftJoin('orders_status','orders_status.id','=','orders.order_status')
                ->where(['orders.id'=>$id])
                ->get();
